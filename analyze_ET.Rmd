...
data_20_25<-data_20_25[, names(all_file)[1:28]]

all_file<-all_file[,1:28]
...
# breakdown fixation prediction by PE trial
agg_wide_all_agg$fixation_prediction_1500_2500_PElow<-unlist(all_tw %>%
                                                filter(time_window == "1500_2500" & type == "low") %>%
                                                group_by(participant ) %>%
                                                summarize(fixation_prediction =
                                                mean(fixation_prediction, na.rm=T))%>%
                                                select(fixation_prediction))

agg_wide_all_agg$fixation_prediction_1500_2500_PEmedium<-unlist(all_tw %>%
                                                filter(time_window == "1500_2500" & type == "medium") %>%
                                                group_by(participant ) %>%
                                                summarize(fixation_prediction =
                                                mean(fixation_prediction, na.rm=T))%>%
                                                select(fixation_prediction))

agg_wide_all_agg$fixation_prediction_1500_2500_PEhigh<-unlist(all_tw %>%
                                                filter(time_window == "1500_2500" & type == "high") %>%
                                                group_by(participant ) %>%
                                                summarize(fixation_prediction =
                                                mean(fixation_prediction, na.rm=T))%>%
                                                select(fixation_prediction))
# now 2000 - 2500
agg_wide_all_agg$fixation_prediction_2000_2500_PElow<-unlist(all_tw %>%
                                                filter(time_window == "2000_2500" & type == "low") %>%
                                                group_by(participant ) %>%
                                                summarize(fixation_prediction =
                                                mean(fixation_prediction, na.rm=T))%>%
                                                select(fixation_prediction))

agg_wide_all_agg$fixation_prediction_2000_2500_PEmedium<-unlist(all_tw %>%
                                                filter(time_window == "2000_2500" & type == "medium") %>%
                                                group_by(participant ) %>%
                                                summarize(fixation_prediction =
                                                mean(fixation_prediction, na.rm=T))%>%
                                                select(fixation_prediction))

agg_wide_all_agg$fixation_prediction_2000_2500_PEhigh<-unlist(all_tw %>%
                                                filter(time_window == "2000_2500" & type == "high") %>%
                                                group_by(participant ) %>%
                                                summarize(fixation_prediction =
                                                mean(fixation_prediction, na.rm=T)) %>%
                                                select(fixation_prediction))
...
#  # delete the x axis text and ticks, as we have the legend
  ...
  ggplot(all_tw %>%
         group_by(participant,time_window, type)%>%
  ...
  library(lme4)
  library(lmerTest)
  mod<-lmer(fixation_prediction~time_window + (1| participant), data=all_tw)
